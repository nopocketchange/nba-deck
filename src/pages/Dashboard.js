    ...
    <div>
      <div className={classNames("p-20", `bg-${dashtheme.background}`)}>
        ...
        <div className={classNames("bg-white p-6 rounded-lg shadow-lg flex ",`bg-${dashtheme.title}`)}>
          ...
          <div className="flex"><Title style={{width:'50px'}}>{team ? favoriteTeam.team  : ""}</Title>   
          <img src={favoriteTeam.img} alt={favoriteTeam.img}  style={{width:'250px'}} /></div>
          <p className={classNames(" text-center", `text-${dashtheme.text}`)}>This is my cool new card!</p>
        </div>
      </div>
      ...
      <p>EST 🕛:{teamDetails["0"]["0"][0]["YEARFOUNDED"]} </p>
      <p>HomeCourt 🏠 :{teamDetails["0"]["0"][0]["ARENA"]}</p>
      <p>Rings:🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆🏆</p>
...
{teamDetails?<Timeline
  dataSource={{
    sourceType:'profile',
    
    screenName: teamDetails.twitter
  }}
  options={{
    height: '600'
  }}
